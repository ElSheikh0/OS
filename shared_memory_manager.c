	                ...
	                if (isWritable == 1)
	                   {
							perm |= PERM_WRITEABLE;
	                                ...
	                                map_frame(myenv->env_page_directory,ptr_frame_info,(uint32)(virtual_address + (i * PAGE_SIZE))
,perm);
					add_frame_to_storage(allocatedObject->framesStorage, ptr_frame_info, i);
		        }
				}
...
int getSharedObject(int32 ownerID, char* shareName, void* virtual_address)
{
	struct Env* myenv = curenv;
		...
		if (ShareObjIndex != E_SHARED_MEM_NOT_EXISTS) {
			...
			struct Share *object_shere = &shares[ShareObjIndex];


			int frames = ROUNDUP(object_shere->size,PAGE_SIZE) / PAGE_SIZE;
		//	uint32 va=(uint32)virtual_address;
			for (int i = 0; i < frames ; i++)
			{
				struct FrameInfo* ptr_frame_info = get_frame_from_storage(
						object_shere->framesStorage, i);
				int perm = PERM_PRESENT | PERM_USER;
				if (object_shere->isWritable == 1) {
					perm = perm | PERM_WRITEABLE;
				}
				//va += ( PAGE_SIZE);
				map_frame(myenv->env_page_directory, ptr_frame_info
						,(uint32)(virtual_address + (i * PAGE_SIZE)), perm);

			}

			object_shere->references++;
